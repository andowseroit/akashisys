create unique index if not exists sales_session_local_id_unique on public.sales(session_id, local_id) where local_id is not null;
create unique index if not exists payments_session_local_id_unique on public.payments(session_id, local_id) where local_id is not null;
create unique index if not exists returns_session_local_id_unique on public.returns(session_id, local_id) where local_id is not null;
create unique index if not exists expenses_session_local_id_unique on public.expenses(session_id, local_id) where local_id is not null;
...
drop trigger if exists validate_financial_insert on public.sales;
create trigger validate_financial_insert before insert on public.sales for each row execute function public.validate_financial_insert();
drop trigger if exists validate_financial_insert on public.payments;
create trigger validate_financial_insert before insert on public.payments for each row execute function public.validate_financial_insert();
drop trigger if exists validate_financial_insert on public.returns;
create trigger validate_financial_insert before insert on public.returns for each row execute function public.validate_financial_insert();
drop trigger if exists validate_financial_insert on public.expenses;
create trigger validate_financial_insert before insert on public.expenses for each row execute function public.validate_financial_insert();
