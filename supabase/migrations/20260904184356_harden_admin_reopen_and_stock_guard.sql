...
-- Additive production hardening. Historical migrations/data are not rewritten.

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'route_session_admin') THEN
    CREATE ROLE route_session_admin NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT NOLOGIN NOBYPASSRLS;
  END IF;
END
$$;
...
AS $$
BEGIN
  -- No custom GUC or client-controllable override exists. The exceptional
  -- transition is recognized only while the dedicated NOLOGIN definer role is
  -- the effective PostgreSQL user.
  ...
  IF OLD.status = 'completed' THEN
    IF NEW.status <> OLD.status
      ...
      RAISE EXCEPTION 'A completed route session is immutable' USING ERRCODE = 'P0001';
    END IF;
    RETURN NEW;
  END IF;
  ...
  IF OLD.status = 'pending' AND NEW.status = 'active' THEN
    IF NEW.started_at IS NULL OR NEW.completed_at IS NOT NULL THEN
      RAISE EXCEPTION 'Starting a route session requires started_at and no completed_at' USING ERRCODE = 'P0001';
    END IF;
  ELSIF OLD.status = 'active' AND NEW.status = 'paused' THEN
    IF NEW.started_at IS DISTINCT FROM OLD.started_at OR NEW.completed_at IS NOT NULL THEN
      RAISE EXCEPTION 'Pausing a route session cannot alter its start time' USING ERRCODE = 'P0001';
    END IF;
  ELSIF OLD.status = 'paused' AND NEW.status = 'active' THEN
    IF NEW.started_at IS DISTINCT FROM OLD.started_at OR NEW.completed_at IS NOT NULL THEN
      RAISE EXCEPTION 'Resuming a route session cannot alter its start time' USING ERRCODE = 'P0001';
    END IF;
  ELSIF (OLD.status = 'active' OR OLD.status = 'paused') AND NEW.status = 'completed' THEN
    IF OLD.started_at IS NULL OR NEW.started_at IS DISTINCT FROM OLD.started_at OR NEW.completed_at IS NULL THEN
      RAISE EXCEPTION 'Completing a route session requires its original start time and a completion time' USING ERRCODE = 'P0001';
    END IF;
    IF EXISTS (
      SELECT 1 FROM public.truck_loads tl
      WHERE tl.session_id = OLD.id
        AND (
          tl.quantity_loaded
          - COALESCE((SELECT SUM(s.quantity) FROM public.sales s WHERE s.session_id = OLD.id AND s.product_id = tl.product_id AND s.voided_at IS NULL), 0)
      ...
      RAISE EXCEPTION 'Cannot complete route session while stock reconciliation has an over-sold or over-returned discrepancy' USING ERRCODE = 'P0001';
    END IF;
  ELSIF OLD.status <> NEW.status THEN
    RAISE EXCEPTION 'Invalid route session status transition: % -> %', OLD.status, NEW.status USING ERRCODE = 'P0001';
  ELSIF NEW.started_at IS DISTINCT FROM OLD.started_at OR NEW.completed_at IS DISTINCT FROM OLD.completed_at THEN
    RAISE EXCEPTION 'Route session timestamps cannot be edited outside a valid state transition' USING ERRCODE = 'P0001';
  END IF;
  RETURN NEW;
END;
$$;
...
BEGIN
  ...
  IF p_target_status = 'pending' THEN
    IF private.route_session_has_nonvoided_financial(p_session_id) THEN
      RAISE EXCEPTION 'Cannot reset a completed session to pending while non-voided financial records exist; reopen it as active instead' USING ERRCODE = 'P0001';
    END IF;
    IF private.route_session_has_truck_loads(p_session_id) THEN
      RAISE EXCEPTION 'Cannot reset a completed session to pending while truck-load stock history exists; reopen it as active instead' USING ERRCODE = 'P0001';
    END IF;
    v_new_started := NULL;
  ELSE
  ...
  SET status = p_target_status,
      ...
      notes = COALESCE(notes, '') || chr(10)
        || '[Admin reopen: ' || p_target_status || ' by ' || p_admin_id::text || ' at ' || now()::text || ']'
  WHERE id = p_session_id;

  INSERT INTO public.corrections(
    table_name, record_id, action, old_values, new_values, corrected_by, reason
  ) VALUES (
    'route_sessions', p_session_id, 'edit',
  ...
  RETURN jsonb_build_object('success', true, 'session_id', p_session_id,
    'previous_status', v_session.status, 'new_status', p_target_status,
    'financial_records_preserved', true);
END;
$$;

GRANT route_session_admin TO postgres;
...
CREATE POLICY route_session_admin_select ON public.route_sessions FOR SELECT TO route_session_admin USING (true);
DROP POLICY IF EXISTS route_session_admin_update ON public.route_sessions;
CREATE POLICY route_session_admin_update ON public.route_sessions FOR UPDATE TO route_session_admin USING (true) WITH CHECK (true);
...
BEGIN
  ...
  SELECT tl.quantity_loaded INTO v_loaded
  FROM public.truck_loads tl
  WHERE tl.session_id = NEW.session_id AND tl.product_id = NEW.product_id
  FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'No truck load exists for this session and product' USING ERRCODE = 'P0001';
  END IF;
  SELECT COALESCE(SUM(s.quantity), 0) INTO v_sold
  FROM public.sales s
  WHERE s.session_id = NEW.session_id AND s.product_id = NEW.product_id AND s.voided_at IS NULL;
  SELECT COALESCE(SUM(r.quantity), 0) INTO v_returned
  FROM public.returns r
  WHERE r.session_id = NEW.session_id AND r.product_id = NEW.product_id AND r.voided_at IS NULL;
  IF v_sold + v_returned + NEW.quantity > v_loaded THEN
    RAISE EXCEPTION 'Insufficient truck stock: loaded %, already sold %, returned %, requested %', v_loaded, v_sold, v_returned, NEW.quantity USING ERRCODE = 'P0001';
  END IF;
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER guard_sale_stock BEFORE INSERT ON public.sales FOR EACH ROW EXECUTE FUNCTION public.guard_sale_stock();
CREATE UNIQUE INDEX IF NOT EXISTS truck_loads_session_product_key ON public.truck_loads(session_id, product_id);
