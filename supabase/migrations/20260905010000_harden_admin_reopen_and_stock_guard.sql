...
-- PostgreSQL requires the executing role to be a member of the new owner role.
-- postgres receives that membership only for this ownership transfer and it is
...
BEGIN
  ...
  IF v_sold + v_returned + NEW.quantity > v_loaded THEN
    RAISE EXCEPTION 'Insufficient truck stock: loaded %, already sold %, returned %, requested %',
      v_loaded, v_sold, v_returned, NEW.quantity USING ERRCODE = 'P0001';
  END IF;
