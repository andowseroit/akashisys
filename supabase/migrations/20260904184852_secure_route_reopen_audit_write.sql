BEGIN;
CREATE OR REPLACE FUNCTION private.record_route_session_reopen_audit(p_session_id uuid,p_admin_id uuid,p_old_status text,p_new_status text,p_old_started_at timestamptz,p_new_started_at timestamptz,p_old_completed_at timestamptz) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $$ BEGIN INSERT INTO public.corrections(table_name,record_id,action,old_values,new_values,corrected_by,reason) VALUES ('route_sessions',p_session_id,'edit',jsonb_build_object('status',p_old_status,'started_at',p_old_started_at,'completed_at',p_old_completed_at),jsonb_build_object('status',p_new_status,'started_at',p_new_started_at,'completed_at',NULL),p_admin_id::text,'Administrative route-session recovery; financial records preserved'); END $$;
REVOKE ALL ON FUNCTION private.record_route_session_reopen_audit(uuid,uuid,text,text,timestamptz,timestamptz,timestamptz) FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION private.record_route_session_reopen_audit(uuid,uuid,text,text,timestamptz,timestamptz,timestamptz) TO route_session_admin;
...
CREATE OR REPLACE FUNCTION public.admin_reopen_route_session(p_session_id uuid,p_admin_id uuid,p_target_status text DEFAULT 'active') RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $$ DECLARE v_session public.route_sessions; v_new_started timestamptz; BEGIN
IF p_admin_id IS NULL THEN RAISE EXCEPTION 'Administrator identity is required' USING ERRCODE='P0001'; END IF;
IF NOT private.is_admin_id(p_admin_id) THEN RAISE EXCEPTION 'Admin access required' USING ERRCODE='P0001'; END IF;
...
PERFORM private.record_route_session_reopen_audit(p_session_id,p_admin_id,v_session.status,p_target_status,v_session.started_at,v_new_started,v_session.completed_at);
